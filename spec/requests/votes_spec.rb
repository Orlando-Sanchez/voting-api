...
describe 'Votes API', type: :request do
    describe 'POST /polls/:poll_id/vote' do
        ...
        it 'returns an error when poll option doesn\'t belong to current poll' do
            user = first_user
            headers = { 'Accept' => 'application/json', 'Content-Type' => 'application/json' }
            auth_headers = Devise::JWT::TestHelpers.auth_headers(headers, user)
            ...
            post "/api/v1/polls/#{poll.id}/vote", params: {
                vote: {poll_id: poll.id, user_id: first_user.id},
                poll_option_id: second_poll.poll_options.first.id
            }.to_json, headers: auth_headers

            expect(response).to have_http_status(:unprocessable_entity)
            expect(response_body).to eq(
                {
                  'message'=> 'The poll option doesn\'t belong to current poll.'
                }
            )           
        end
